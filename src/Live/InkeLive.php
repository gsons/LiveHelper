...
class InkeLive extends Live
{
    ...
    public function getLiveUrl($roomId)
    {
        ...
        if (isset($data['data']['live_addr'][0]['rtmp_stream_addr'])) {
            return $data['data']['live_addr'][0]['rtmp_stream_addr'];
        } else {
    ...
    public function getDancingRoom()
    {
        ...
        if (isset($data['cards']) && !empty($data['cards'])) {
            $list = $data['cards'];
            foreach ($list as $vo) {
                if (!isset($vo['data']['live_info']['creator']['id'])) continue;
                ...
                if ($isDancing1 || $isDancing2) {
                    $arr[] = ['roomId' => intval($vo['data']['live_info']['creator']['id']), 'nickName' => str_replace(' ', '', $vo['data']['live_info']['creator']['nick'])];
                }
            }
    ...
    function getHotDanceRoom()
    {
        ...
        if (isset($data['cards']) && !empty($data['cards'])) {
            ...
            foreach ($list as $vo) {
                if (!isset($vo['data']['live_info']['creator']['id'])) continue;
                $room_id = intval($vo['data']['live_info']['creator']['id']);
                $nick_name = str_replace(' ', '', $vo['data']['live_info']['creator']['nick']);
                $time = time();
                $arr[] = [
                    ...
                    'nick_name' => $nick_name,
                    'room_id' => $room_id,
                    'room_url' => sprintf(self::BASE_ROOM_URL, $room_id),
                    'record_date' => date('Y-m-d H:i:s', $time),
                    'record_time' => $time,
                    'hot_num' => $vo['data']['live_info']['online_users']
                ];
            }
        }
        unset($data);
